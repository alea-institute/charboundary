...
def convert_segmenter_model(model_getter, model_name: str, output_path: Optional[str] = None, 
    ...
    if optimization_level not in [0, 1, 2, 3]:
        print(f"Error: Invalid optimization level {optimization_level}. Must be between 0-3.")
        return False
...
def convert_custom_model(input_path: str, output_path: str, feature_count: Optional[int] = None,
                        optimization_level: int = 2) -> bool:
    """
...
def convert_all_models(optimization_levels: Optional[Dict[str, int]] = None) -> bool:
    ...
    if optimization_levels is None:
        optimization_levels = {name: info["optimal_level"] for name, info in BUILTIN_MODELS.items()}
...
def benchmark_model_optimization_levels(model, feature_count: int, model_name: str, 
    ...
    successful_levels = {k: v for k, v in results["onnx_results"].items() 
...
def benchmark_built_in_models(runs: int = 20, batch_size: int = 500) -> Dict[str, Any]:
    ...
    for model_name, model_info in BUILTIN_MODELS.items():
        ...
        print(f"\n{'='*70}")
        print(f"Benchmarking {model_name.upper()} model with optimization level {model_info['optimal_level']}")
        print(f"{'='*70}")
        ...
        try:
            ...
            onnx_time, onnx_predictions = run_onnx_benchmark(model, data, runs, model_info["optimal_level"])
            onnx_ops = (runs * batch_size) / onnx_time
            speedup = onnx_ops / sklearn_ops
            ...
            accuracy_indicator = "✓" if predictions_match else "✗"
            print(f"ONNX (level {model_info['optimal_level']}): {onnx_ops:.2f} ops/sec, {onnx_time:.4f} seconds")
    ...
    try:
        ...
        try:
            if hasattr(segmenter.model, "enable_onnx"):
                ...
                print(f"✓ Called enable_onnx with optimization level {optimization_level}")
        except Exception as e:
