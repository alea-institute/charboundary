#!/usr/bin/env python3
"""
Script to test the medium model for the CharBoundary library.
"""
...
def test_medium_model() -> Tuple[int, int]:
    """
    Test the medium model for the CharBoundary library.
    
    Returns:
        Tuple[int, int]: (number of successful tests, number of failed tests)
    """
    print("CharBoundary Medium Model Test")
    print("==============================")
    ...
    model_paths = [
        os.path.join(model_dir, "medium_model.skops"),
        os.path.join(model_dir, "medium_model.skops.xz"),
        os.path.join(model_dir, "medium_model.skops.lzma")
    ]
    ...
    for path in model_paths:
        if os.path.exists(path):
            found_model = True
            model_path = path
            print(f"Found medium model at {path} ({os.path.getsize(path) / (1024*1024):.1f} MB)")
    ...
    if not found_model:
        print(f"Error: Medium model not found at any of the expected locations:")
        for path in model_paths:
            print(f"  - {path}")
        print("Please run scripts/train_medium_model.py first to generate the model.")
        return 0, 1
    
    # Load the model
    print("\nLoading the medium model...", end="")
    t0 = time.time()
    try:
        ...
        print(f"\nError loading the medium model: {e}")
        return 0, 1
    ...
    print("\nUsage example:")
    example_text = "This is a simple example. It demonstrates how to use the medium model."
...
if __name__ == "__main__":
    successes, failures = test_medium_model()
